...
int nAnonymizeCasinocoinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "casinocoin" is a composite category enabling all CasinoCoin-related debug output
            if (ptrCategory->count(string("casinocoin"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty casinocoin.conf if it does not exist
